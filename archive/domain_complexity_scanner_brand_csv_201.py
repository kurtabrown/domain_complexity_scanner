...
v2.0.1-no-browser+retail

Key changes vs 2.0.0:
- Count ANY HTTP response or curl-discovered redirect as "active"
- Add portal/e-commerce signal (+3) using light probes to common paths
- Add ecosystem hub bump (+2) for apexes with >=3 subdomains listed
- Keep Included logic, inactive penalty, flexible CSV, curl+HTML/JS hints
"""

import argparse, csv, os, sys, logging, re, unicodedata, shutil, subprocess, time
from dataclasses import dataclass, asdict
from typing import List, Optional, Tuple, Dict

print("[Domain Scanner] v2.0.1-no-browser+retail")
...
PORTAL_PATHS = ("/login", "/signin", "/account", "/checkout", "/cart", "/auth")
...
def priority_from_score(score: int) -> str:
    if score >= 12: return "High"   # tightened slightly
...
def analyze_domain(
    ...
    # Non-domain rows → default Medium
    if not looks_like_domain(d):
        reason = "Custom defined search term (not a domain)"
        return DomainResult(d, d, primary, owner_apex, None, "Standalone", "",
                            False, None, [], False, [], "", [], "",
    ...
    http_status = None
    inactive = False
    observed_statuses_pairs: List[Tuple[str,int]] = []
    ...
    if not covered_by_val and check_redirects and remaining() > 0:
        ...
        if curl:
            for start in (f"http://{d}/", f"https://{d}/", f"http://www.{d}/", f"https://www.{d}/"):
                ...
                try:
                    cmd = [curl, "-sS", "-I", "--http1.1", "-L",
                           "-m", str(int(min(5, max(2, http_timeout)))), start]
                    if not HTTP_VERIFY:  # --insecure
                        cmd.insert(2, "-k")
                    out = subprocess.run(cmd, capture_output=True, text=True,
                                         timeout=min(remaining(), http_timeout+2))
                    if out.returncode == 0 and out.stdout:
                        locs = re.findall(r"^Location:\s*(.+)$", out.stdout,
        ...
        # Any curl header activity implies "active"
        if redirect_chain_hosts and not http_present:
            http_present = True
    ...
    # HTML/JS hints (no navigation)
    if not covered_by_val and with_http and remaining() > 0 and httpx is not None:
        tgs, html_exp = detect_html_js_targets(d, timeout=min(http_timeout, remaining()),
                                               verify_tls=HTTP_VERIFY, deadline_ts=deadline_ts)
    ...
    # HTTP presence (quick & forgiving)
    if with_http and remaining() > 0 and httpx is not None:
        for host in (d, "www."+d):
            for scheme in ("https", "http"):
                ...
                try:
                    ...
                    with httpx.Client(follow_redirects=False, timeout=tout,
                        ...
                        observed_statuses_pairs.append((f"{scheme}://{host}/", r.status_code))
                        http_present = True
                        if http_status is None:
    ...
    # Portal / commerce signal via common paths
    portal_signal = False
    if with_http and remaining() > 0 and httpx is not None:
        for host in (d, "www."+d):
            for scheme in ("https", "http"):
                for p in PORTAL_PATHS:
                    ...
                    try:
                        ...
                        with httpx.Client(follow_redirects=False, timeout=tout,
                                          headers={"User-Agent": BROWSER_UA},
                                          verify=HTTP_VERIFY) as client:
                            r = client.get(f"{scheme}://{host}{p}")
                            if r.status_code in (200, 301, 302, 303, 307, 308, 401, 403):
                                portal_signal = True
                                LOG.debug("Portal signal %s://%s%s -> %s", scheme, host, p, r.status_code)
                                break
                    except Exception as e:
                        LOG.debug("Portal probe %s://%s%s -> %s", scheme, host, p, e)
                if portal_signal: break
            if portal_signal: break

    # Ecosystem hub (precomputed in main)
    hub_subdomain_count = hub_counts.get(apex, 0)
    is_hub = hub_subdomain_count >= 3

    # Inactive if no DNS at all
    ...
    score, reasons = 0, []
    if primary:
        score += 8; reasons.append("Primary brand domain (+8)")
    if http_present:
        score += 3; reasons.append("Active HTTP endpoint (+3)")
    if portal_signal:
        score += 3; reasons.append("Portal/e-commerce signal (+3)")
    if is_hub:
        score += 2; reasons.append(f"Ecosystem hub with {hub_subdomain_count} subdomains (+2)")
    if inactive:
        score -= 5; reasons.append("Inactive/unreachable (−5)")
    ...
    observed_statuses = [f"{u}→{s}" for (u,s) in observed_statuses_pairs[:6]]
    ...
    return DomainResult(
        ...
        http_present=http_present, http_status=http_status, observed_statuses=observed_statuses,
        inactive=inactive, redirect_chain_hosts=redirect_chain_hosts, redirect_explain=redirect_explain,
...
def main():
    ...
    # Precompute hub counts (# of listed subdomains per apex)
    hub_counts: Dict[str, int] = {}
    for (d, _, _) in rows:
        if looks_like_domain(d):
            apx = apex_of(d) or d
            if d != apx:
                hub_counts[apx] = hub_counts.get(apx, 0) + 1
    ...
    fields = [
        ...
        "http_present","http_status","observed_statuses","inactive",
        "redirect_chain_hosts","redirect_explain",
        "htmljs_targets","htmljs_explain",
        "signal_priority","signal_score","signal_reason"
    ]

    print(f"Scanning {len(rows)} item(s) with max {args.max_sects_per_domain if hasattr(args,'max_sects_per_domain') else args.max_secs_per_domain}s/domain…\n")
    ...
    with open(out_path, "w", encoding="utf-8", newline="") as fh:
        ...
        for (d, prim, owner) in rows:
            res = analyze_domain(
                ...
                hub_counts=hub_counts, with_http=args.with_http, check_redirects=args.check_redirects,
            ...
            row["observed_statuses"] = ",".join(res.observed_statuses)
            w.writerow(row)
            ...
            if args.explain and d in (args.explain or []):
                ...
                if res.observed_statuses:
                    print(f" - HTTP statuses: {', '.join(res.observed_statuses)}")
