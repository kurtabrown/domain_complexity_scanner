...
# -*- coding: utf-8 -*-
"""
Domain Complexity Scanner (v3.1.2)

Change:
- NEW: Early keyword detection. If an input value is not a probable domain,
       classify as Medium with reason "Custom defined search term (non-domain)"
       and skip DNS/HTTP/CT/breach checks.

All prior v3.1.1 behavior preserved.
...
def is_probable_domain(s: str) -> bool:
    """
    Conservative check: must have a dot, and tldextract must find a domain+suffix.
    Rejects tokens like 'bjs-gen-prod' or 'search term'.
    """
    h = normalize_host(s)
    if "." not in h:
        return False
    if tldextract:
        try:
            ...
            return False
    # Fallback: minimal pattern like example.com or a.b.co.uk
    return bool(re.fullmatch(r"[a-z0-9-]+(\.[a-z0-9-]+)+", h))
...
def detect_redirect_chain(host: str, insecure: bool, paths: Optional[List[str]] = None, max_hops: int = 8, timeout: float = 6.0):
    ...
    verify = not insecure
    chain: List[str] = []
    client = httpx.Client(timeout=timeout, verify=verify, headers={"User-Agent": f"domain-scanner/{VERSION}"})
    try:
...
def is_included_by_coverage(host: str, monitored_apexes: Set[str], redirect_target: Optional[str]) -> Tuple[bool, Optional[str], bool, bool]:
    ...
    Rules:
      ...
      4) If a subdomain redirects to its (covered) apex, it's Included via (1).
    """
    h = normalize_host(host)
    a = apex_of(h)
    ...
    if redirect_target:
        ...
        # Same-apex/self redirect -> ignore for apex; subdomains already covered by (1)
        if rt_apex == a:
            if h == a:
...
def main():
    ...
    if args.with_http or args.check_redirects:
        http_client = httpx.Client(
            ...
            headers={"User-Agent": f"domain-scanner/{VERSION}"}
        )
    ...
    try:
        for r in rows:
            ...
            # === NEW: non-domain keyword handling (early exit) ===
            if not is_probable_domain(host):
                apex = host  # keep as-is for transparency
                res = Result(host=host, apex=apex)
                res.signal_priority = "Medium"
                res.priority_reason = "Custom defined search term (non-domain). Skipped DNS/HTTP checks."
                # Write and continue to next row
                if args.explain and (host in [normalize_host(h) for h in args.explain]):
                    print(f"\n[Explain] host={host}")
                    print(f" - apex(host)={apex}")
                    print(f" - classified_as=Medium (custom search term)")
                    print(f" - reason=Custom defined search term (non-domain). Skipped DNS/HTTP checks.")
                results.append(res)
                continue
            # === END NEW ===
